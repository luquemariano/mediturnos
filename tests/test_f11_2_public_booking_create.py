...
def slot(day=0, hour=10, minute=30):
...
def payload(s, **paciente): return {"prestacion":s.identificador_publico,"fecha_hora":slot(),"paciente":{"nombre":"Ana","apellido":"Pérez","email":" ANA@EXAMPLE.COM ",**paciente}}
...
def test_datetime_naive_rechazado(client):
    db=SessionTest(); p,s=escenario(db); assert client.post(f"/public/profesionales/{p.slug_publico}/reservas",json={**payload(s),"fecha_hora":slot()[:-6]}).status_code==422; db.close()
...
def test_rechaza_slot_fuera_de_disponibilidad_y_menor_a_dos_horas(client,monkeypatch):
    db=SessionTest(); p,s=escenario(db); ahora=BASE_NOW; monkeypatch.setattr("app.services.public_booking_service.ahora_negocio",lambda:ahora)
...
def test_reutiliza_por_dni_y_no_sobrescribe_maestro(client):
    ...
    data=payload(s,dni=" 12345678 "); assert client.post(f"/public/profesionales/{p.slug_publico}/reservas",json=data).status_code==201
...
def test_anticipacion_exacta_permitida_y_una_minuto_menor_rechazada(client, monkeypatch):
    ...
    body=payload(s); body["fecha_hora"]=slot(hour=10,minute=0); assert client.post(f"/public/profesionales/{p.slug_publico}/reservas",json=body).status_code==201
    body["fecha_hora"]=slot(hour=9,minute=59); assert client.post(f"/public/profesionales/{p.slug_publico}/reservas",json=body).status_code in (400,409); db.close()

def test_mas_de_60_dias_rechaza_sin_efectos(client, monkeypatch):
    db=SessionTest(); p,s=escenario(db); monkeypatch.setattr("app.services.public_booking_service.ahora_negocio",lambda:BASE_NOW); body=payload(s); body["fecha_hora"]=slot(day=61,hour=10,minute=0); antes=db.query(Paciente).count(); assert client.post(f"/public/profesionales/{p.slug_publico}/reservas",json=body).status_code==400; assert db.query(Paciente).count()==antes; db.close()

def test_franja_extraordinaria_permite_reserva(client, monkeypatch):
    db=SessionTest(); p,s=escenario(db); monkeypatch.setattr("app.services.public_booking_service.ahora_negocio",lambda:BASE_NOW); fecha=BASE_DATE+timedelta(days=4)
    db.add(DisponibilidadExcepcion(profesional_id=p.id,fecha=fecha,tipo="franja_extraordinaria",origen="manual",hora_inicio=time(10),hora_fin=time(11),activa=True)); db.commit(); body=payload(s); body["fecha_hora"]=slot(day=4,hour=10,minute=0); assert client.post(f"/public/profesionales/{p.slug_publico}/reservas",json=body).status_code==201; db.close()

def test_cierre_de_dia_bloquea_disponibilidad_habitual(client, monkeypatch):
    db=SessionTest(); p,s=escenario(db); monkeypatch.setattr("app.services.public_booking_service.ahora_negocio",lambda:BASE_NOW); fecha=BASE_DATE+timedelta(days=4)
    db.add(Disponibilidad(profesional_id=p.id,dia_semana=4,hora_inicio=time(10),hora_fin=time(11))); db.add(DisponibilidadExcepcion(profesional_id=p.id,fecha=fecha,tipo="cierre_dia",origen="manual",activa=True)); db.commit(); body=payload(s); body["fecha_hora"]=slot(day=4,hour=10,minute=0); assert client.post(f"/public/profesionales/{p.slug_publico}/reservas",json=body).status_code in (400,409); db.close()
...
def test_vinculo_unico_turno_cancelado_y_uuid_estable(client):
    db=SessionTest(); p,s=escenario(db); db.add(Disponibilidad(profesional_id=p.id,dia_semana=1,hora_inicio=time(10),hora_fin=time(13))); db.commit(); body=payload(s); body["fecha_hora"]=slot(); first=client.post(f"/public/profesionales/{p.slug_publico}/reservas",json=body); assert first.status_code==201; rid=first.json()["reserva_id"]; body["fecha_hora"]=slot(hour=11,minute=30); second=client.post(f"/public/profesionales/{p.slug_publico}/reservas",json=body); assert second.status_code==201; assert db.query(ProfesionalPaciente).count()==1; turno=db.query(Turno).filter(Turno.identificador_publico==rid).one(); assert UUID(turno.identificador_publico).version==4; assert turno.identificador_publico==rid
    turno.estado="cancelado"; db.commit(); body["fecha_hora"]=slot(); assert client.post(f"/public/profesionales/{p.slug_publico}/reservas",json=body).status_code==201; db.close()
