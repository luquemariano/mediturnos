...
def test_notifications_are_isolated_and_readable(client):
    ...
    whatsapp = next(item for item in payload["items"] if item["type"] == "product_release" and item["entity_id"] == 129)
    assert whatsapp["title"] == "Nuevo: recordatorios automáticos por WhatsApp"
    assert "confirmen o cancelen" in whatsapp["message"]
    assert whatsapp["read_at"] is None
    assert payload["unread_count"] == sum(item["read_at"] is None for item in payload["items"])
...
def test_product_release_is_idempotent_per_professional():
    ...
    first = release(items, 127); second = release(items, 128); whatsapp = release(items, 129)
    assert len(first) == 1 and len(second) == 1 and len(whatsapp) == 1 and unread == 3
    assert first[0].title == "Nueva función: Lista de espera inteligente"
    assert second[0].title == "Nuevo: compartí tu página de reservas"
    assert whatsapp[0].title == "Nuevo: recordatorios automáticos por WhatsApp"
    assert "Paciente" not in first[0].message and "Paciente" not in second[0].message and "Paciente" not in whatsapp[0].message
    other_items, other_unread = list_notifications(db, other_user.id)
    assert len(release(other_items, 127)) == 1 and len(release(other_items, 128)) == 1 and len(release(other_items, 129)) == 1 and other_unread == 3
    assert {item.user_id for item in first + second + whatsapp} == {user.id}
    assert {item.user_id for item in release(other_items, 127) + release(other_items, 128) + release(other_items, 129)} == {other_user.id}
    ...
    assert release(items_after, 128)[0].read_at is None and release(items_after, 129)[0].read_at is None and unread_after == 2
    mark_notification_read(db, user.id, second[0].id)
    mark_notification_read(db, user.id, whatsapp[0].id)
    items_final, unread_final = list_notifications(db, user.id)
    assert release(items_final, 127)[0].read_at is not None and release(items_final, 128)[0].read_at is not None and release(items_final, 129)[0].read_at is not None and unread_final == 0
    assert len(release(items_final, 127)) == 1 and len(release(items_final, 128)) == 1 and len(release(items_final, 129)) == 1
