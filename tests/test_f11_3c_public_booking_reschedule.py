...
def slot(day=0, hour=10, minute=0):
    return f"{(BASE_DATE + timedelta(days=day)).isoformat()}T{hour:02d}:{minute:02d}:00-03:00"
...
def test_reprogramar_reservado_conserva_identidad_y_token(client, monkeypatch):
    ...
    response=client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot(hour=11)})
    ...
    assert turno.fecha_hora != turno.fecha_fin and client.get(f"/public/reservas/{token}").json()["fecha_hora"]==slot(hour=11)
    db.close()
...
@pytest.mark.parametrize("fecha,esperado",[(slot(),200),(slot(hour=9,minute=59),400),(slot(day=61),400)])
def test_politica_temporal_publica(client, monkeypatch, fecha, esperado):
    ahora=BASE_NOW; monkeypatch.setattr("app.services.public_booking_service.ahora_negocio",lambda: ahora)
...
def test_datetime_naive_es_rechazado(client):
    db, _, _, reserva=preparar(client); assert client.post(f"/public/reservas/{reserva['autogestion_token']}/reprogramar",json={"fecha_hora":slot(hour=11)[:-6]}).status_code==422; db.close()
...
def test_conflicto_no_muta_turno_original(client, monkeypatch):
    ...
    otro_body=payload(prestacion,email="ocupante@example.com"); otro_body["fecha_hora"]=slot(hour=11); assert client.post(f"/public/profesionales/{profesional.slug_publico}/reservas",json=otro_body).status_code==201
    original=(turno.fecha_hora,turno.fecha_fin,turno.estado,turno.identificador_publico,turno.autogestion_token_hash)
    assert client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot(hour=11)}).status_code==409
...
def test_disponibilidad_antes_y_despues_de_reprogramar(client, monkeypatch):
    ...
    assert slot() not in before and slot(minute=30) in before
    assert client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot(minute=30)}).status_code==200
    after=client.get(f"/public/profesionales/{profesional.slug_publico}/disponibilidad",params=params).json()["dias"][0]["horarios"]
    assert slot() in after and slot(minute=30) not in after
    db.close()
...
def test_doble_reprogramacion_y_duracion(client, monkeypatch):
    ...
    for hora in (slot(minute=30),slot(hour=11)):
        assert client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":hora}).status_code==200
    data=client.get(f"/public/reservas/{token}").json(); assert data["fecha_hora"]==slot(hour=11) and data["fecha_fin"]==slot(hour=11,minute=30); db.close()
...
def test_misma_fecha_hora_es_idempotente(client, monkeypatch):
    ...
    response=client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot()})
    assert response.status_code==200 and response.json()["fecha_hora"]==slot(); db.close()
...
def test_cierre_de_dia_rechaza_sin_mutar(client, monkeypatch):
    ...
    db.add(DisponibilidadExcepcion(profesional_id=profesional.id,fecha=BASE_DATE,tipo="cierre_dia",origen="manual",activa=True)); db.commit()
    assert client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot(hour=11)}).status_code in (400,409); db.refresh(turno); assert turno.fecha_hora==original; db.close()
...
def test_franja_extraordinaria_permite_reprogramar(client, monkeypatch):
    ...
    db.add(DisponibilidadExcepcion(profesional_id=profesional.id,fecha=BASE_DATE,tipo="franja_extraordinaria",origen="manual",hora_inicio=time(12),hora_fin=time(13),activa=True)); db.commit()
    assert client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot(hour=12)}).status_code==200; db.close()
...
def test_sqlite_naive_y_privacidad(client, monkeypatch):
    ...
    response=client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot(hour=11)}); assert response.status_code==200; assert response.headers["cache-control"]=="no-store" and response.headers["referrer-policy"]=="no-referrer"; assert "autogestion_token" not in response.json(); db.close()
...
def test_rate_limit_reprogramacion_10_11_y_buckets_independientes(client, monkeypatch):
    ...
    assert client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot()}).status_code==429
    ...
    body=payload(prestacion,email="bucket-independiente@example.com"); body["fecha_hora"]=slot(hour=11)
...
def test_invariantes_completas_exitosas_y_duracion(client, monkeypatch):
    ...
    assert client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot(hour=11)}).status_code==200; db.refresh(turno)
...
def test_payload_extra_rechazado(client, campo):
    db, _, _, reserva=preparar(client); payload_extra={"fecha_hora":slot(hour=11),campo:1}; assert client.post(f"/public/reservas/{reserva['autogestion_token']}/reprogramar",json=payload_extra).status_code==422; db.close()
...
def test_estados_no_reprogramables(client, estado):
    ...
    assert client.post(f"/public/reservas/{token}/reprogramar",json={"fecha_hora":slot(hour=11)}).status_code in (400,409)
    db.close()

def test_reprogramar_token_invalido_y_payload_extra(client):
    assert client.post("/public/reservas/no-existe/reprogramar",json={"fecha_hora":slot(hour=11)}).status_code==404
    db, profesional, prestacion, reserva=preparar(client)
    assert client.post(f"/public/reservas/{reserva['autogestion_token']}/reprogramar",json={"fecha_hora":slot(hour=11),"estado":"cancelado"}).status_code==422
    db.close()
