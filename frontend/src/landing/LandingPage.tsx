...
const iconos = ["calendar", "users", "clock", "calendar", "briefcase", "mail-message", "clock", "devices"];
...
function LineIcon({ name }: { name: string }) {
  const paths: Record<string, React.ReactNode> = {
    ...
    "mail-message": <><rect x="2.5" y="5" width="13" height="10" rx="2"/><path d="m3.5 6 5.5 4 5.5-4"/><path d="M16 10h3a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-5l-3 2v-4"/></>,
  };
...
export default function LandingPage() {
  ...
  return <div className="landing-page" onClick={medirRegistro}>
    ...
    <main>
      ...
      <section id="profesionales" className="landing-section audience"><div className="landing-container"><div className="section-heading"><p className="eyebrow">Para quién es</p><h2>Pensado para profesionales que gestionan su propia consulta</h2><p>Conocé también <a href="/para-psicopedagogos">Turnelia para psicopedagogos</a>.</p></div><div className="audience__list">{["Médicos", "Psicólogos", "Psiquiatras", "Psicopedagogos", "Nutricionistas", "Kinesiólogos", "Otros profesionales independientes"].map(item => <span key={item}>{item}</span>)}</div></div></section>
      <FAQ />
      <section id="contacto" className="dark-cta dark-cta--final"><div className="landing-container dark-cta__inner"><div><h2>Menos tiempo organizando. Más tiempo para tus pacientes.</h2><p>Empezá a ordenar tu consulta con Turnelia.</p><div className="landing-contacto"><a href="mailto:info@turnelia.com.ar">info@turnelia.com.ar</a><a href={WHATSAPP_URL}>+54 9 351 227 7416</a></div></div><div className="button-row"><a className="button button--light" href="/registro">Probar Turnelia</a>{WHATSAPP_URL ? <a className="button button--dark-outline" href={WHATSAPP_URL} target="_blank" rel="noreferrer">Hablar por WhatsApp</a> : <button className="button button--dark-outline" disabled title="Canal de WhatsApp pendiente de configurar">Hablar por WhatsApp</button>}</div></div></section>
    </main>
    <footer className="landing-footer"><div className="landing-container landing-footer__grid"><div><img src="/brand/mediturnos-logo-horizontal.svg" alt="Turnelia"/><p>Agenda y gestión profesional.</p></div><div><h2>Producto</h2><a href="#funciones">Funciones</a><a href="#precios">Precios</a><a href="#demo">Demo</a></div><div><h2>Turnelia</h2><a href="#contacto">Contacto</a><a href="/ayuda">Centro de Ayuda</a><a href="/privacidad">Privacidad</a><a href="/terminos">Términos</a></div><div><h2>Acceso</h2><a href="/login">Ingresar</a></div></div><div className="landing-container landing-footer__bottom">© 2026 Turnelia. Todos los derechos reservados.</div></footer>
  </div>;
}
