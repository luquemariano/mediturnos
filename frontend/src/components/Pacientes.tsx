  ...
  return <ProfesionalShell activo="pacientes" nombre={props.nombre} tituloTopbar="Pacientes"
    ...
    <div className="pacientes-pagina">
      ...
      : <ul className="pacientes-lista">{pacientes.map((paciente) => <li key={paciente.id}>
          ...
          <dl><div><dt>Teléfono</dt><dd>{paciente.telefono || "No informado"}</dd></div><div><dt>Email</dt><dd>{paciente.email || "No informado"}</dd></div><div><dt>DNI</dt><dd>{paciente.dni || "No informado"}</dd></div><div className="pacientes-whatsapp"><dt>WhatsApp</dt><dd><span className={`pacientes-whatsapp-badge${paciente.whatsapp_opt_in ? " activo" : ""}`}>{paciente.whatsapp_opt_in ? "Activo" : "Sin autorización"}</span>{paciente.whatsapp_opt_in && paciente.whatsapp_opt_in_at && <small>Autorizado {fechaEvolucion(paciente.whatsapp_opt_in_at).split(" · ")[0]}</small>}</dd></div></dl>
    ...
    {seleccion && <><button type="button" className="paciente-detalle-fondo" aria-label="Cerrar detalle" onClick={() => setSeleccion(null)} />
      <aside className="paciente-detalle" aria-label={`Detalle de ${seleccion.nombre} ${seleccion.apellido}`}>
        ...
        <PatientStudyRequests patientId={seleccion.id} initialStudyRequestId={props.studyRequestIdInicial} />
    ...
    {modal && <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="paciente-modal-titulo"><form className="modal-paciente" onSubmit={guardar}>
      ...
      <section className="whatsapp-form-section" aria-labelledby="whatsapp-form-title"><div className="whatsapp-form-heading"><div><h3 id="whatsapp-form-title">WhatsApp</h3><p>Recordatorios, confirmaciones y cancelaciones de turnos.</p></div></div><label className="whatsapp-consent-label"><input type="checkbox" checked={form.whatsapp_opt_in} onChange={(e) => setForm({ ...form, whatsapp_opt_in: e.target.checked })}/><span>El paciente autorizó recibir comunicaciones relacionadas con sus turnos por WhatsApp.</span></label><div className="whatsapp-phone-help"><strong>Teléfonos aceptados</strong><p>Podés ingresar 0351 15..., 351... o +54 9 351.... Turnelia lo normaliza automáticamente.</p><small>Si el número empieza sólo con 15, necesitás incluir también el código de área.</small></div></section>
