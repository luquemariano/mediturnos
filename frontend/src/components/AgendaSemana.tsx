...
type Props = { turnos: Turno[]; fecha: FechaCivil; ahora: Date; onSeleccionarDia: (fecha: FechaCivil) => void; onSeleccionarTurno: (turno: Turno) => void };
...
export default function AgendaSemana({ turnos, fecha, ahora, onSeleccionarDia, onSeleccionarTurno }: Props) {
  ...
  return <section className="agenda-semana" aria-label={`Semana ${formatearSemana(fecha)}`}>
    ...
    <div className="agenda-semana-rango">{formatearSemana(fecha)}</div>
    <div className="agenda-semana-grid" style={{ "--filas": filas.length, "--alto": `${(fin - inicio) * ESCALA}px` } as CSSProperties}>
      ...
      {dias.map((dia) => <div key={dia} className={`agenda-semana-columna${dia === hoy ? " es-hoy" : ""}${dia === fecha ? " es-seleccionada" : ""}`}>
        <button type="button" className="agenda-semana-dia" onClick={() => onSeleccionarDia(dia)} aria-label={`Ver ${formatearFechaCivil(dia)}`}>{formatearFechaCivil(dia)}{dia === hoy && <small>Hoy</small>}</button>
