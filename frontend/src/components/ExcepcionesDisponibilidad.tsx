...
import { cerrarMiDisponibilidadPorRango, crearMiExcepcion, crearMiFeriado, eliminarMiExcepcion, eliminarMiFeriado, obtenerMisExcepciones, reabrirMiDisponibilidadPorRango } from "../services/disponibilidadService";
...
function agruparExcepciones(items: DisponibilidadExcepcion[]): Array<DisponibilidadExcepcion | GrupoCierres> {
  const cierres = items.filter((item) => item.tipo === "cierre_dia" && (item.origen === "vacaciones" || item.origen === "legacy")).sort((a, b) => a.fecha.localeCompare(b.fecha));
  ...
  return [...grupos, ...items.filter((item) => item.tipo === "franja_extraordinaria" || (item.origen !== "vacaciones" && item.origen !== "legacy"))]
...
export default function ExcepcionesDisponibilidad() {
  ...
  const [feriadoAbierto, setFeriadoAbierto] = useState(false);
  const [tipoFeriado, setTipoFeriado] = useState<"feriado" | "no_laborable">("feriado");
  ...
  async function confirmarEliminar() {
    ...
    setGuardando(true); setError("");
    try {
      const esFeriado = eliminando.origen === "feriado" || eliminando.origen === "no_laborable";
      await (esFeriado ? eliminarMiFeriado(eliminando.id) : eliminarMiExcepcion(eliminando.id));
      setItems((actuales) => actuales.filter((item) => item.id !== eliminando.id));
      setExito(esFeriado ? "Feriado eliminado correctamente." : eliminando.tipo === "cierre_dia" ? "Fecha reabierta correctamente." : "Horario especial eliminado correctamente.");
      setEliminando(null);
    } catch (e) { setError(mensajeError(e, "No pudimos eliminar la excepción.")); }
    finally { setGuardando(false); }
  }

  async function guardarFeriado(evento: FormEvent) {
    evento.preventDefault();
    if (guardando) return;
    if (!fecha) { setError("Seleccioná una fecha."); return; }
    setGuardando(true); setError("");
    try {
      const creado = await crearMiFeriado({ fecha, tipo: tipoFeriado, ...(nombreFeriado.trim() ? { nombre: nombreFeriado.trim() } : {}) });
      setItems((actuales) => [...actuales, creado]);
      setFeriadoAbierto(false);
      setExito(tipoFeriado === "feriado" ? "Feriado agregado correctamente." : "Día no laborable agregado correctamente.");
    } catch (e) { setError(mensajeError(e, "No pudimos agregar la fecha no laborable.")); }
    finally { setGuardando(false); }
  }
  ...
  async function confirmarReabrirGrupo() {
    ...
    try {
      ...
      setItems((actuales) => actuales.filter((item) => (item.origen !== "vacaciones" && item.origen !== "legacy") || item.fecha < grupoReabrir.fecha_desde || item.fecha > grupoReabrir.fecha_hasta));
  ...
  return <section className="mi-excepciones" aria-labelledby="mi-excepciones-titulo">
    <header><div><span>Excepciones por fecha</span><h2 id="mi-excepciones-titulo">Cambios puntuales</h2><p>Usá excepciones para cerrar un día puntual o abrir un horario especial sin modificar tu semana habitual.</p></div><div className="mi-excepciones-acciones"><button type="button" onClick={() => abrir("cierre_dia")}>Cerrar un día</button><button type="button" onClick={() => { setVacacionesAbiertas(true); setDesde(""); setHasta(""); setError(""); setExito(""); }}>Cargar vacaciones</button><button type="button" onClick={() => { setFeriadoAbierto(true); setFecha(""); setTipoFeriado("feriado"); setNombreFeriado(""); setError(""); setExito(""); }}>Agregar feriado</button><button type="button" onClick={() => abrir("franja_extraordinaria")}>Agregar horario especial</button></div></header>
    ...
    {cargando ? <div className="mi-excepciones-cargando" aria-label="Cargando excepciones"><span /><span /></div>
      ...
      : <ol>{visuales.map((item) => "fecha" in item ? <li key={`e-${item.id}`} className={item.origen === "feriado" || item.origen === "no_laborable" ? "mi-excepciones-feriado" : undefined}><i className={item.origen} aria-hidden="true" /><div><strong>{fechaLegible(item.fecha)}</strong><small>{item.tipo === "franja_extraordinaria" ? "Horario especial" : item.origen === "feriado" ? "Feriado" : item.origen === "no_laborable" ? "Día no laborable" : "Día cerrado"}</small>{item.nombre && <span className="mi-excepciones-motivo">{item.nombre}</span>}</div>{item.tipo === "franja_extraordinaria" && <time>{item.hora_inicio?.slice(0, 5)}–{item.hora_fin?.slice(0, 5)}</time>}<button type="button" onClick={() => { setEliminando(item); setError(""); }}>{item.tipo === "franja_extraordinaria" ? "Eliminar horario" : item.origen === "feriado" || item.origen === "no_laborable" ? "Quitar feriado" : "Reabrir fecha"}</button></li> : <li key={`g-${item.fecha_desde}`} className="mi-excepciones-periodo"><i className="cierre_dia" aria-hidden="true" /><div><strong>{item.fecha_desde === item.fecha_hasta ? fechaLegible(item.fecha_desde) : `${fechaLegible(item.fecha_desde)} — ${fechaLegible(item.fecha_hasta)}`}</strong><small>{item.items.length === 1 ? "Día de vacaciones" : `Vacaciones · ${item.items.length} días`}</small></div><button type="button" onClick={() => { item.items.length === 1 ? setEliminando(item.items[0]) : setGrupoReabrir(item); setError(""); }}>{item.items.length === 1 ? "Reabrir fecha" : "Reabrir período"}</button></li>)}</ol>}

    {feriadoAbierto && <div className="mi-excepciones-modal-fondo"><section role="dialog" aria-modal="true" aria-labelledby="mi-excepciones-feriado-titulo" className="mi-excepciones-modal"><span>Fecha no laborable</span><h3 id="mi-excepciones-feriado-titulo">Agregar feriado</h3><form onSubmit={guardarFeriado}><label>Fecha<input aria-label="Fecha del feriado" type="date" min={fechaActualNegocio()} value={fecha} disabled={guardando} onChange={(e) => setFecha(e.target.value)} /></label><label>Tipo<select aria-label="Tipo de fecha" value={tipoFeriado} disabled={guardando} onChange={(e) => setTipoFeriado(e.target.value as "feriado" | "no_laborable")}><option value="feriado">Feriado</option><option value="no_laborable">Día no laborable</option></select></label><label>Nombre o motivo <small>Opcional</small><input aria-label="Nombre o motivo" maxLength={120} value={nombreFeriado} disabled={guardando} onChange={(e) => setNombreFeriado(e.target.value)} /></label><p className="mi-excepciones-aviso">Ese día no ofrecerá horarios habituales para nuevas reservas. <strong>Los turnos ya creados para esta fecha no serán cancelados.</strong></p>{error && <p role="alert" className="mi-excepciones-feedback error">{error}</p>}<div className="mi-excepciones-modal-acciones"><button type="button" disabled={guardando} onClick={() => setFeriadoAbierto(false)}>Volver</button><button type="submit" disabled={guardando}>{guardando ? "Guardando…" : "Confirmar"}</button></div></form></section></div>}
    ...
    {eliminando && <div className="mi-excepciones-modal-fondo"><section role="dialog" aria-modal="true" aria-labelledby="mi-excepciones-eliminar-titulo" className="mi-excepciones-modal"><span>Excepción por fecha</span><h3 id="mi-excepciones-eliminar-titulo">{eliminando.origen === "feriado" || eliminando.origen === "no_laborable" ? "Quitar feriado" : eliminando.tipo === "cierre_dia" ? "Reabrir esta fecha" : "Eliminar horario especial"}</h3><p className="mi-excepciones-resumen"><strong>{fechaLegible(eliminando.fecha)}</strong>{eliminando.tipo === "franja_extraordinaria" && <><br />{eliminando.hora_inicio?.slice(0, 5)}–{eliminando.hora_fin?.slice(0, 5)}</>}</p><p>{eliminando.origen === "feriado" || eliminando.origen === "no_laborable" ? "Sólo se quitará esta marca. Si existe otro cierre para la fecha, permanecerá cerrada." : "Este cambio sólo afecta los próximos horarios disponibles. Los turnos existentes permanecen sin cambios."}</p>{error && <p role="alert" className="mi-excepciones-feedback error">{error}</p>}<div className="mi-excepciones-modal-acciones"><button type="button" disabled={guardando} onClick={() => setEliminando(null)}>Volver</button><button type="button" className="eliminar" disabled={guardando} onClick={() => void confirmarEliminar()}>{guardando ? "Eliminando…" : eliminando.origen === "feriado" || eliminando.origen === "no_laborable" ? "Quitar feriado" : eliminando.tipo === "cierre_dia" ? "Reabrir fecha" : "Eliminar horario"}</button></div></section></div>}
  </section>;
}
