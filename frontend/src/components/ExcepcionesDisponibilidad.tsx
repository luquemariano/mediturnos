...
import { crearMiExcepcion, eliminarMiExcepcion, obtenerMisExcepciones } from "../services/disponibilidadService";
...
export default function ExcepcionesDisponibilidad() {
  ...
  return <section className="mi-excepciones" aria-labelledby="mi-excepciones-titulo">
    ...
    {cargando ? <div className="mi-excepciones-cargando" aria-label="Cargando excepciones"><span /><span /></div>
      ...
      : <ol>{ordenadas.map((item) => <li key={item.id}><i className={item.tipo} aria-hidden="true" /><div><strong>{fechaLegible(item.fecha)}</strong><small>{item.tipo === "cierre_dia" ? "Día cerrado" : "Horario especial"}</small></div>{item.tipo === "franja_extraordinaria" && <time>{item.hora_inicio?.slice(0, 5)}–{item.hora_fin?.slice(0, 5)}</time>}<button type="button" onClick={() => { setEliminando(item); setError(""); }}>{item.tipo === "cierre_dia" ? "Reabrir fecha" : "Eliminar horario"}</button></li>)}</ol>}
