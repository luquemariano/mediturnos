...
export default function SelfService({ token }: { token: string }) {
  ...
  return <main className="public-booking"><section className="public-card"><p className="public-kicker">Turnelia</p><h1>Tu reserva</h1><h2>{r.profesional.nombre} {r.profesional.apellido}</h2><p>{r.prestacion.nombre} · {r.prestacion.modalidad}</p><p>{new Date(r.fecha_hora).toLocaleString("es-AR", { dateStyle: "full", timeStyle: "short" })}</p><strong>{r.estado}</strong>{mensajeExito && <p role="status">{mensajeExito}</p>}{mensajeError && <p role="alert">{mensajeError}</p>}{["reservado", "confirmado"].includes(r.estado) && <><label>Nueva fecha<input type="date" value={nuevaFecha} onChange={e => void consultarDisponibilidad(e.target.value)} /></label>{cargandoDisponibilidad ? <p>Cargando horarios disponibles…</p> : <fieldset><legend>Horarios disponibles</legend>{horarios.length ? horarios.map(h => <button type="button" key={h} disabled={h === r.fecha_hora} className={horaSeleccionada === h ? "selected" : ""} onClick={() => setHoraSeleccionada(h)}>{new Date(h).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" })}</button>) : nuevaFecha && <p>No hay horarios disponibles para este día.</p>}</fieldset>}<button type="button" onClick={reprogramar} disabled={!horaSeleccionada}>Reprogramar</button><button type="button" onClick={cancelar}>Cancelar turno</button></>}</section></main>;
}
