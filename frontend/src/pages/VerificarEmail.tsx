...
export default function VerificarEmail({ onLogin }: { onLogin: () => void }) {
  ...
  useEffect(() => { const token = new URLSearchParams(window.location.search).get("token"); if (!token) { setEstado("invalido"); return; } void verificarEmail(token).then(() => { setEstado("verificado"); capturePostHogEvent("email_verified", { source: "email_verification" }); }).catch((error) => { if (axios.isAxiosError(error) && error.response?.status === 400) setEstado("invalido"); else setEstado("invalido"); }); }, []);
