...
import reservaOnline from "./content/reserva-online.md?raw";
import reservaOnlinePaciente from "./content/reserva-online-paciente.md?raw";
import listaDeEspera from "./content/lista-de-espera.md?raw";
import compartirReservas from "./content/como-compartir-pagina-reservas.md?raw";
...
const ARTICLES: HelpArticle[] = [parseArticle(primerosPasos), parseArticle(prestaciones), parseArticle(disponibilidad), parseArticle(agenda), parseArticle(turnos), parseArticle(pacientes), parseArticle(historiaClinica), parseArticle(documentos), parseArticle(estudios), parseArticle(recordatorios), parseArticle(suscripcion), parseArticle(reservaOnline), parseArticle(reservaOnlinePaciente), parseArticle(listaDeEspera), parseArticle(compartirReservas)].sort((a, b) => a.order - b.order);
