...
vi.mock("../src/api/api", () => ({ default: { get: vi.fn().mockResolvedValue({ data: [{ id: 1, nombre: "Clínica", descripcion: null, duracion_turno_minutos: 30, activa: true }] }), post: vi.fn() } }));
...
describe("registro profesional", () => {
  ...
  it("persiste el resultado mediante el callback luego del registro", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Nombre"),{target:{value:"Ana"}});fireEvent.change(screen.getByLabelText("Apellido"),{target:{value:"Pérez"}});fireEvent.change(screen.getByLabelText("Correo electrónico"),{target:{value:"ana@test.com"}});fireEvent.change(screen.getByLabelText(/Contraseña/),{target:{value:"secreto123"}});fireEvent.change(screen.getByLabelText("Matrícula"),{target:{value:"MP100"}});fireEvent.change(await screen.findByLabelText("Especialidad"),{target:{value:"1"}});fireEvent.click(screen.getByRole("button",{name:"Crear cuenta"}));
