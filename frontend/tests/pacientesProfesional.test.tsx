...
describe("Pacientes profesional",()=>{
  ...
  it("emite patient_created una vez al crear",async()=>{vi.mocked(servicio.crearPacienteProfesional).mockResolvedValue(paciente);render(<Pacientes {...props}/>);await screen.findByText("No hay pacientes para mostrar.");fireEvent.click(screen.getByText("Nuevo paciente"));fireEvent.change(screen.getByLabelText("Nombre *"),{target:{value:"Ana"}});fireEvent.change(screen.getByLabelText("Apellido *"),{target:{value:"López"}});fireEvent.click(screen.getByText("Crear paciente"));await waitFor(()=>expect(analytics.capturePostHogEvent).toHaveBeenCalledTimes(1));expect(analytics.capturePostHogEvent).toHaveBeenCalledWith("patient_created",{source:"patients"});});
  it("no emite patient_created al editar",async()=>{vi.mocked(servicio.buscarPacientesProfesional).mockResolvedValue([paciente]);vi.mocked(servicio.obtenerHistorialPaciente).mockResolvedValue([]);vi.mocked(servicio.editarPacienteProfesional).mockResolvedValue(paciente);render(<Pacientes {...props}/>);fireEvent.click(await screen.findByText("Ver paciente"));fireEvent.click(screen.getAllByText("Editar")[0]);fireEvent.change(screen.getByLabelText("Nombre *"),{target:{value:"Ana nueva"}});fireEvent.change(screen.getByLabelText("Apellido *"),{target:{value:"López"}});fireEvent.click(screen.getByText("Guardar cambios"));await waitFor(()=>expect(servicio.editarPacienteProfesional).toHaveBeenCalled());expect(analytics.capturePostHogEvent).not.toHaveBeenCalled();});
