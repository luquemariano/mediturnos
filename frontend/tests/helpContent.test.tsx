...
describe("catálogo de ayuda", () => {
  it("carga artículos únicos y conserva las guías esenciales", () => { const articles = getHelpArticles(); expect(articles.map((a) => a.order)).toEqual([...articles].map((a) => a.order).sort((a, b) => a - b)); expect(new Set(articles.map((a) => a.slug)).size).toBe(articles.length); expect(articles.map((a) => a.slug)).toEqual(expect.arrayContaining(["primeros-pasos", "reserva-online", "agenda", "lista-de-espera", "como-compartir-pagina-reservas"])); expect(getHelpArticleBySlug("lista-de-espera")?.title).toBe("Cómo usar la lista de espera"); expect(getHelpArticleBySlug("como-compartir-pagina-reservas")?.title).toBe("Cómo compartir tu página de reservas"); });
  it("busca, filtra y navega por slug", () => { expect(getHelpArticleBySlug("primeros-pasos")?.title).toBe("Primeros pasos"); expect(getHelpArticleBySlug("no-existe")).toBeUndefined(); expect(getHelpArticlesByCategory("configuracion")).toHaveLength(2); expect(getHelpArticlesByCategory("pacientes")).toHaveLength(1); expect(getHelpArticlesByCategory("clinica")).toHaveLength(3); expect(getHelpArticlesByCategory("agenda").length).toBeGreaterThanOrEqual(4); expect(getHelpArticlesByCategory("cuenta")).toHaveLength(1); expect(getPreviousHelpArticle("prestaciones")?.slug).toBeTruthy(); expect(getNextHelpArticle("turnos")?.slug).toBe("pacientes"); expect(getNextHelpArticle("recordatorios")?.slug).toBe("suscripcion"); });
