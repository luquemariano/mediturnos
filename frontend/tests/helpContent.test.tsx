...
describe("catálogo de ayuda", () => {
  ...
  it("contiene guías funcionales sin el placeholder inicial", () => { const primeros = getHelpArticleBySlug("primeros-pasos")?.body ?? ""; const prestaciones = getHelpArticleBySlug("prestaciones")?.body ?? ""; expect(primeros).toContain("## 1. Crear la cuenta"); expect(primeros).toContain("/ayuda/prestaciones"); expect(primeros).not.toContain("sujeto a ampliación"); expect(prestaciones).toContain("## Crear una prestación"); expect(prestaciones).toContain("## Desactivar una prestación"); expect(prestaciones).toContain("turnos ya creados"); });
