...
describe("catálogo de ayuda", () => {
  it("carga los trece artículos ordenados y sin slugs duplicados", () => { const articles = getHelpArticles(); expect(articles.map((a) => a.order)).toEqual([10, 12, 13, 20, 30, 40, 50, 60, 70, 80, 90, 100, 110]); expect(articles.map((a) => a.slug)).toEqual(["primeros-pasos", "reserva-online", "reserva-online-paciente", "prestaciones", "disponibilidad", "agenda", "turnos", "pacientes", "historia-clinica", "documentos", "estudios", "recordatorios", "suscripcion"]); expect(new Set(articles.map((a) => a.slug)).size).toBe(articles.length); });
  it("busca, filtra y navega por slug", () => { expect(getHelpArticleBySlug("primeros-pasos")?.title).toBe("Primeros pasos"); expect(getHelpArticleBySlug("no-existe")).toBeUndefined(); expect(getHelpArticlesByCategory("configuracion")).toHaveLength(2); expect(getHelpArticlesByCategory("pacientes")).toHaveLength(1); expect(getHelpArticlesByCategory("clinica")).toHaveLength(3); expect(getHelpArticlesByCategory("agenda")).toHaveLength(4); expect(getHelpArticlesByCategory("cuenta")).toHaveLength(1); expect(getPreviousHelpArticle("prestaciones")?.slug).toBe("reserva-online-paciente"); expect(getNextHelpArticle("turnos")?.slug).toBe("pacientes"); expect(getNextHelpArticle("pacientes")?.slug).toBe("historia-clinica"); expect(getNextHelpArticle("historia-clinica")?.slug).toBe("documentos"); expect(getNextHelpArticle("documentos")?.slug).toBe("estudios"); expect(getNextHelpArticle("estudios")?.slug).toBe("recordatorios"); expect(getNextHelpArticle("recordatorios")?.slug).toBe("suscripcion"); });
