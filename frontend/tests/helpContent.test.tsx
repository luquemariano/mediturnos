...
describe("catálogo de ayuda", () => {
  it("carga los siete artículos ordenados y sin slugs duplicados", () => { const articles = getHelpArticles(); expect(articles.map((a) => a.order)).toEqual([10, 20, 30, 40, 50, 60, 70]); expect(articles.map((a) => a.slug)).toEqual(["primeros-pasos", "prestaciones", "disponibilidad", "agenda", "turnos", "pacientes", "historia-clinica"]); expect(new Set(articles.map((a) => a.slug)).size).toBe(articles.length); });
  it("busca, filtra y navega por slug", () => { expect(getHelpArticleBySlug("primeros-pasos")?.title).toBe("Primeros pasos"); expect(getHelpArticleBySlug("no-existe")).toBeUndefined(); expect(getHelpArticlesByCategory("configuracion")).toHaveLength(2); expect(getHelpArticlesByCategory("pacientes")).toHaveLength(1); expect(getHelpArticlesByCategory("clinica")).toHaveLength(1); expect(getPreviousHelpArticle("prestaciones")?.slug).toBe("primeros-pasos"); expect(getNextHelpArticle("prestaciones")?.slug).toBe("disponibilidad"); expect(getNextHelpArticle("turnos")?.slug).toBe("pacientes"); expect(getNextHelpArticle("pacientes")?.slug).toBe("historia-clinica"); });
  ...
  it("contiene guías funcionales sin el placeholder inicial", () => { const primeros = getHelpArticleBySlug("primeros-pasos")?.body ?? ""; const prestaciones = getHelpArticleBySlug("prestaciones")?.body ?? ""; const disponibilidad = getHelpArticleBySlug("disponibilidad")?.body ?? ""; const agenda = getHelpArticleBySlug("agenda")?.body ?? ""; const turnos = getHelpArticleBySlug("turnos")?.body ?? ""; const pacientes = getHelpArticleBySlug("pacientes")?.body ?? ""; const clinica = getHelpArticleBySlug("historia-clinica")?.body ?? ""; expect(primeros).toContain("## 1. Crear la cuenta"); expect(primeros).toContain("/ayuda/prestaciones"); expect(primeros).not.toContain("primera versión"); expect(prestaciones).toContain("## Crear una prestación"); expect(prestaciones).toContain("## Desactivar una prestación"); expect(prestaciones).toContain("turnos ya creados"); expect(disponibilidad).toContain("## Varias franjas en un mismo día"); expect(disponibilidad).toContain("no cancela automáticamente"); expect(agenda).toContain("## Vista Día"); expect(agenda).toContain("## Vista Semana"); expect(agenda).toContain("## Vista Mes"); expect(agenda).toContain("No existe una acción manual profesional llamada"); expect(turnos).toContain("## Crear un turno"); expect(turnos).toContain("## Reprogramar un turno"); expect(turnos).toContain("## Marcar como ausente"); expect(turnos).toContain("## Finalizar un turno"); expect(pacientes).toContain("## Crear un paciente"); expect(pacientes).toContain("## Desactivar un paciente"); expect(pacientes).toContain("Historial de turnos"); expect(clinica).toContain("## Resumen clínico"); expect(clinica).toContain("## Agregar una evolución"); expect(clinica).toContain("La evolución queda asociada al profesional"); expect(clinica).toContain("no se edita ni se reemplaza"); expect(clinica).toContain("## Quién puede acceder"); });
