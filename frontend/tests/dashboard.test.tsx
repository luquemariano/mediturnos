...
function renderizar(rol: string) {
  const accion = vi.fn();
  render(<Dashboard nombre="Usuario" rol={rol} onAbrirPacientes={accion} onAbrirProfesionales={accion} onAbrirEspecialidades={accion} onAbrirPrestaciones={accion} onAbrirTurnos={accion} onAbrirDisponibilidades={accion} onAbrirPerfil={accion} onCerrarSesion={accion} />);
}

describe("dashboard por rol", () => {
  it("muestra todos los módulos administrativos al administrador", () => {
    renderizar("administrador");
    for (const nombre of ["Pacientes", "Profesionales", "Especialidades", "Prestaciones", "Turnos", "Disponibilidades"]) expect(screen.getByRole("button", { name: new RegExp(nombre) })).toBeInTheDocument();
  });
